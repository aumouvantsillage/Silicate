...
(define signals-tests
  (test-suite "Signals"
    (test-case "Constant signal asserts const~? predicate"
      (check-pred const~? (const~ 42)))
    ...
    (test-case "Can create a signal from a list"
      (letrec ([l (range 1 5)]
               [s (list->signal l)])
        (check-equal? l (take~ s (length l)))
        (check-pred const~? (drop~ s (sub1 (length l))))))
    ...
    (test-case "Can add1 to a signal"
      (letrec ([l (range 1 5)]
        ...
        (check-equal? (map add1 l) (take~ s (length l)))))

    (test-case "Can sub1 to a signal"
      (letrec ([l (range 1 5)]
               [s (sub1~ (list->signal l))])
        (check-equal? (map sub1 l) (take~ s (length l)))))
    ...
    (test-case "Can add signals"
      (letrec ([l1 (range 1   5   1)]
        ...
        (check-equal? (map + l1 l2 l3) (take~ s (length l1)))))

    (test-case "Can negate a signal"
      (letrec ([l (range 1 5)]
               [s (-~ (list->signal l))])
        (check-equal? (map - l) (take~ s (length l)))))
    ...
    (test-case "Can use if on signals"
      (letrec ([l1 (list #f #f #t #t)]
        ...
        (check-equal? (map (lambda (c x y) (if c x y)) l1 l2 l3) (take~ s (length l1)))))

    (test-case "Can register a signal"
      (letrec ([e  (list #f #t #f #t #t #f)]
               [l1 (list 10 20 30 40 50 60)]
               [l2 (list 0  0  20 20 40 50)]
               [s  (register 0 (list->signal e) (list->signal l1))])
        (check-equal? l2 (take~ s (length l2)))))

    (test-case "Can create a simple counter"
      (letrec ([n 10]
               [s (signal 0 (add1~ s))])
        (check-equal? (range n) (take~ s n))))

    (test-case "Can create a modulo counter"
      (letrec ([n1 4] [n2 (* 3 n1)]
               [s (signal 0 (if~ (=~ s (const~ (sub1 n1))) (const~ 0) (add1~ s)))])
        (check-equal? (build-list n2 (lambda (x) (remainder x n1)))
                      (take~ s n2))))

    (test-case "Can cascade counters"
      (letrec ([n1 4] [n2 (* 5 n1)]
               [s1 (signal 0 (if~ e (const~ 0) (add1~ s1)))]
               [e (=~ s1 (const~ (sub1 n1)))]
               [s2 (register 0 e (add1~ s2))])
    ...
    (test-case "Can oversample a signal"
      (letrec ([t1 7] [t2 3]
               ...
               [s2 (cross-domain t1 t2 s1)]
               [l (list 0 1 1 2 2 3 3 3 4 4 5 5 6 6 6)])
        (check-equal? l (take~ s2 (length l)))))
