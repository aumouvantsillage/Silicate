      ...
      (begin-silicate
        (module
          ...
          (component D
            ...
            (instance c (multiplicity (literal-expr 2)) C (literal-expr 10))
            (assignment (field-expr (indexed-expr (name-expr c) (literal-expr 0)) a) (name-expr x))
            (assignment (field-expr (indexed-expr (name-expr c) (literal-expr 1)) a) (name-expr y))
            (assignment (name-expr z) (call-expr + (field-expr (indexed-expr (name-expr c) (literal-expr 0)) b)
                                                   (field-expr (indexed-expr (name-expr c) (literal-expr 1)) b))))))
      (define d (make-instance-D))
      (define d-x (list->signal (list 10 20 30 40 50)))
